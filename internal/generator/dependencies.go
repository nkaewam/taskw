...
func (g *DependencyGenerator) generateDependencyFileContent(providersByPackage map[string][]scanner.ProviderFunction, imports []string) (string, error) {
	...
	tmpl, err := template.New("dependencies").Parse(dependencyTemplate)
	if err != nil {
...
// dependencyTemplate is the Go template for generating the dependencies file
const dependencyTemplate = `// Code generated by taskw. DO NOT EDIT.
...
{{- range $pkg, $providers := .ProvidersByPackage}}

	// {{$pkg}} module providers
{{- range $providers}}
	{{call $.GetProviderRef $pkg .FunctionName}},
{{- end}}
{{- end}}
)
`
